...
public class BoardDao {
	...
	public List<Board> selectByPage(int pageNo, int rowsPerPage) throws SQLException{
		List<Board> list = new ArrayList<>();
		String sql="";
		sql+=" 	aselect rn, bno,btitle,bcontent,bwriter,bhitcount,bdate ";
		sql+=" from(select rownum as rn,bno,btitle,bcontent,bwriter,bhitcount,bdate ";
		sql+=" FROM (select bno, btitle,bcontent,bwriter,bhitcount,bdate from board order by bno asc) ";
		sql+=" where rownum <=?) ";
		sql+=" WHERE rn>=? ";
		PreparedStatement pstmt = conn.prepareStatement(sql);
